...
def _ensure_ntrace_binary() -> Path:
    ...
    try:
        ...
        raise OSError("NextTrace API 查询失败")
...
def _run_mcis_process(
    ...
    return _buffer.decode("utf-8", errors="replace"), result_file


def _parse_mcis_to_verified(
    buffer_text: str, result_file: Path,
) -> tuple[list[tuple[str, str, str, str, str, str]], list[str], dict[str, dict[str, str]]] | None:
    import csv as _csv
    ...
    return _display_rows, _result_lines, _dl_map


def _trace_routes_concurrent(
    display_rows: list[tuple[str, str, str, str, str, str]],
) -> list[tuple[str, str, str, str, str, str]]:
    total = len(display_rows)
    _idx_map: dict[int, str] = {}
...
def step_montecarlo(cfg: ScannerConfig, auto_mcis: bool = False, colo: str = "", colo_exclude: str = "", do_route_trace: bool = True,
    ...
    parsed = _parse_mcis_to_verified(buffer_text, result_file)
    if parsed is None:
        return 0
    ...
    if dl_map:
        _dl_ok = sum(1 for v in dl_map.values() if v["ok"] == "true")
        _dl_total = len(dl_map)
    ...
    if not auto_mcis and display_rows and not download_url:
        _hp_inp = _safe_input("  是否对发现的 IP 进行高端口探测？（8443/2053/2096 等 | Y 确认 | 回车跳过）：", to_lower=True)
        if _hp_inp == "y":
            _hp_rows = _mcis_high_port_scan(display_rows, cfg)
            if _hp_rows:
                print_sep("-", C.B)
                print(c(f"  MCIS 高端口探测结果｜新增 {len(_hp_rows)} 条", C.LC))
                _mcis_hdr2 = ("  " + _pad_cjk("IP 地址", 18, '<') + "  " + _pad_cjk("端口", 8, '<') +
                             "  " + _pad_cjk("延迟(ms)", 8, '<') + "  " + _pad_cjk("速度(MB/s)", 14, '<') +
                             "  " + _pad_cjk("地区码", 8, '<') + "  " + _pad_cjk("所属网段", 16, '<') +
                             "  " + _pad_cjk("线路", 24, '<'))
                print(c(_mcis_hdr2, C.W))
                _new_csv = []
                for _ii, _rrow in enumerate(_hp_rows):
                    _rip, _rport, _rlat, _rspd, _rpref, _rcolo, _rrt = _rrow
                    _rpref_display = _rpref
                    _rt_display = _rrt or "-"
                    _rt_color2 = C.LG if "精品" in _rt_display else (C.LY if "优化" in _rt_display else C.W)
                    _r_color2 = C.LG if _ii == 0 else (C.LY if _ii < 3 else C.W)
                    print(c("  " + _pad_cjk(_rip, 18, '<') + "  " + _pad_cjk(str(_rport), 8, '<') +
                            "  " + _pad_cjk(_rlat, 8, '<') + "  " + _pad_cjk(_rspd or "-", 14, '<') +
                            "  " + _pad_cjk(_rcolo.upper(), 8, '<') + "  " + _pad_cjk(_rpref_display, 16, '<') +
                            "  " + c(_pad_cjk(_rt_display, 24, '<'), _rt_color2), _r_color2))
                    gi = geo_lookup(_rip)
                    _rcountry = gi.get("country", "") if gi else ""
                    _rcity = gi.get("city", "") if gi else ""
                    _rproto = "IPv6" if ":" in _rip else "IPv4"
                    _new_csv.append(f"{_rip},{_rport},TRUE,{_rcolo},{_rcountry},{_rcity},{_rlat},{_rspd},,{_rproto}")
                if _new_csv:
                    existing = verified_file.read_text(encoding="utf-8").strip().split("\n") if verified_file.exists() else []
                    if existing and existing[0] == CSV_HEADER:
                        existing = existing[1:]
                    seen = set()
                    for _el in existing:
                        _ek = _el.split(",")[0] if _el else ""
                        if _ek:
                            seen.add(_ek)
                    for _nl in _new_csv:
                        _nip = _nl.split(",")[0]
                        if _nip not in seen:
                            existing.append(_nl)
                            seen.add(_nip)
                    verified_file.write_text(CSV_HEADER + "\n" + "\n".join(existing) + "\n")
                total_count += len(_hp_rows)
...
MCIS_HIGH_PORTS = [8443, 2053, 2083, 2087, 2096, 2052, 2082, 2086, 2095, 8880, 8080, 9443]


def _mcis_high_port_scan(
    display_rows: list[tuple[str, str, str, str, str, str]],
    cfg: ScannerConfig,
) -> list[tuple[str, str, str, str, str, str, str]]:
    ips = list(dict.fromkeys(r[0] for r in display_rows))
    if not ips:
        return []
    print(c(f"  [HPS] 高端口探测: {len(ips)} IP x {len(MCIS_HIGH_PORTS)} 端口", C.CY))

    found: list[tuple[str, int]] = []
    total = len(ips) * len(MCIS_HIGH_PORTS)
    done = 0
    with ThreadPoolExecutor(max_workers=min(50, len(ips) * len(MCIS_HIGH_PORTS))) as ex:
        fmap: dict[Any, tuple[str, int]] = {}
        for ip in ips:
            for port in MCIS_HIGH_PORTS:
                fmap[ex.submit(quick_probe, ip, port, 2)] = (ip, port)
        for f in as_completed(fmap):
            ip, port = fmap[f]
            done += 1
            try:
                ...
                pass
            write_progress(done / total * 100, f" | 高端口探活 ({done}/{total})")
    write_progress_done(f" | 高端口探活完成 | 发现 {len(found)} 个开放端口")

    if not found:
        print(c("  [HPS] 未发现开放高端口", C.LY))
        return []

    _tag = f"mcis_hp_{os.getpid()}"
    masscan_file = BASE / f".{_tag}_masscan.txt"
    hits_file = BASE / f".{_tag}_hits.txt"
    verified_out = BASE / f".{_tag}_verified.txt"

    masscan_file.write_text("\n".join(f"{ip}:{port}" for ip, port in found) + "\n")

    hits = 0
    try:
        ensure_cf_scanner()
        subprocess.run(
            [str(CF_SCANNER), "-i", str(masscan_file), "-o", str(hits_file),
             "-c", "100", "-connect-timeout", "3s"],
        ...
        if hits:
            subprocess.run([
                sys.executable, str(VERIFY_PY),
                "--input", str(hits_file),
                "--output", str(verified_out),
                "--api", API_URL,
                "--chunk", "500", "--concurrent", "32",
            ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=600)

    except Exception as e:
        print(c(f"  [HPS] 验证失败: {e}", C.LY))
        return []

    new_display: list[tuple[str, str, str, str, str, str, str]] = []
    if verified_out.exists():
        with open(verified_out) as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith("#") or line.startswith("IP"):
                    continue
                ...
                try:
                    ...
                    ck = ""
                route = _trace_route(ip)
                new_display.append((ip, port, lat, spd, ck, colo, route))

    for f in (masscan_file, hits_file, verified_out):
        try:
    ...
    if not new_display:
        print(c("  [HPS] 验证后无可用结果", C.LY))
        return []

    print(c(f"  [HPS] 高端口验证通过: {len(new_display)} 条", C.G))
    return new_display
...
def main() -> None:
    ...
    parser.add_argument("--mcis-url", metavar="URL", type=str, default="",
                        help="MCIS 自定义测速文件地址 (默认: 使用目标 IP 的 /__down?bytes=50000000)")
